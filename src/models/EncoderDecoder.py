...
class EncoderDecoder(LightningModule):
    ...
    def __init__(self, config, tokenizer, transformer, dataset_reader):
        ...
        self.save_hyperparameters(ignore=['tokenizer','transformer','dataset_reader'])
    ...
    def validation_epoch_end(self, outputs):
        ...
        if not (self.use_deepspeed or self.use_ddp) or dist.get_rank() == 0:
            ...
            self.log("acc",metrics["accuracy"])
            for key, value in accumulated.items():
                if key.startswith("log."):
                    metrics[key.replace("log.", "")] = mean(value)
            print("@",self.global_step,"@",type(self.global_step))
            result_str = str(self.global_step)+json.dumps(metrics) + "\n"
    ...
    def load_model(self):
        if self.config.load_weight == "":
            return
        if self.config.load_weight[-4:] != "ckpt":
